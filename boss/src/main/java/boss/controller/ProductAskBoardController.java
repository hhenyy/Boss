...
public class ProductAskBoardController {
	...
	public String productAskBoardSelectForm(Model model, AskBoard askboard, int pid, Product product,
			HttpSession session) {
	...
	public String productAskBoardUpdateCheck(Model model, AskBoard askboard, Product product, HttpSession session) {

		int result = 0;

		result = service.askupdate(askboard);
	...
	// 댓글 폼 이동
	@RequestMapping("askReplyForm.do")
	public String askReplyForm(Model model, AskBoard askboard, Product product, HttpSession session) {
		System.out.println("askReplyForm:" + "댓글 달아라~~");
...
//		askboard = service.askselect(askboard.getAskid());
		...
		return "./product/askReply/askReplyForm";
	}
