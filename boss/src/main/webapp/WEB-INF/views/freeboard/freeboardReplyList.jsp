...
$(function() {
	$('.edit1').click(function() {
		var id  = $(this).attr('id');  // rno, this:사용자가 선택한 
		var txt = $('#td_'+id).text(); // replytext, text():text내용을 가져옴 
...
function up(id) { //댓글 내용 수정 
	...
	var formData = "frId="+id+'&replytext='+replytext
		+"&fId=${detail.fId}";
				...
				<c:forEach var="frboard" items="${freplylist}">
					<tr>
						...
						<c:if test="${frboard.frDrop !='Y' }">
							...
							<td id="btn_${frboard.frId}"><c:if
									test="${frboard.mEmail==detail.mEmail}">
